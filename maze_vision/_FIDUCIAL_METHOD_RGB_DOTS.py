...
#### TOOLS ####
class FIDUCIAL_METHOD_RGB_DOTS():
    """
    ...
    def __init__(self, userColorList, userInputList, maskImg_MazeOnly, maze_size_meters, tolerance=100):
        ...
        # Load Feature Data
        self.dots = {}
        self._load_dots(userColorList, userInputList, tolerance)
        ...
        # Process
        self.scale 		  = self._calc_scale(self.dots, maze_size_meters)
        self.rotMatrix 	  = self._calc_rotation(self.dots)

        if self.rotMatrix is not None:
            self.mazeCentroid = self._calc_maze_centroid(self.dots, self.rotMatrix)
            self.path_image_cropped = self._calc_maze_mask(self.dots, maskImg_MazeOnly, self.rotMatrix)
            return
        else:
            return None

    def _load_dots(self, userColorList, userFileList, tolerance=50):
        """
        ...
        for colorName, file in zip(userColorList, userFileList):
            with open(file) as csvfile:
                ...
                print(' > INFO: Found largest <' + str(colorName) + '> pixel group with <' + str(largest_volume) + '> elements.')
    ...
    def _calc_scale(self, dots, maze_size_meters):
        """
        ...
        print(' > INFO: Found maze real/px scale = ' + str(scale))

        return scale

    def _calc_rotation(self, dots):
        """
        ...
        if dot_prod < np.cos(np.deg2rad(45)):
            ...
            print(" > INFO: X & Y axis insufficiently square. Failed to assemble rotation matrix.")
            return None
    ...
    def _calc_maze_centroid(self, dots, rotMatrix):
        """
    ...
    def _calc_maze_mask(self, dots, maskImg_MazeOnly, rotMatrix):
        """
        ...
        print(" > INFO: Four Corners Identified @ :", corner_NW, corner_NE, corner_SW, corner_SE)
        ...
        print(" > INFO: Z-Axis Angle Rotation Correction: " + str(int(np.degrees(rotAngle2D))) + " degrees")
