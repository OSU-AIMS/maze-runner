...
#include <maze_msgs/srv/generate_motion_plan.hpp>
...
static const std::string PROFILE ="MAZERUNNER";
static const std::string PLANNING_SERVICE = "create_motion_plan";
static const std::string TESSERACT_MONITOR_NAMESPACE = "maze_environment";
static const double MAX_TCP_SPEED = 0.25;  // m/s



tesseract_common::Toolpath fromMsg(const geometry_msgs::msg::PoseArray& msg)
{
    tesseract_common::Toolpath tps;
    tesseract_common::VectorIsometry3d seg;

    seg.reserve(msg.poses.size());
...
public:
    ...
    server_ = node_->create_service<maze_msgs::srv::GenerateMotionPlan>(
        PLANNING_SERVICE, std::bind(&PlanningServer::plan, this, std::placeholders::_1, std::placeholders::_2));
    RCLCPP_INFO(node_->get_logger(), "Started the Maze Runner motion planning server");
  }
...
private:
    tesseract_planning::CompositeInstruction createProgram(const tesseract_planning::ManipulatorInfo& info,
                                                            const tesseract_common::Toolpath& path)
    {
        ...
        tesseract_planning::CartesianWaypoint wp1 = path[0][0];
        ...
        // Define Maze Motion Plan
        tesseract_planning::CompositeInstruction maze_motion_plan(PROFILE);
        for (std::size_t i = 1; i < path[0].size(); ++i)
        {
            tesseract_planning::CartesianWaypoint wp = path[0][i];
            maze_motion_plan.push_back(
                tesseract_planning::PlanInstruction(wp, tesseract_planning::PlanInstructionType::LINEAR, PROFILE));
        }
        program.push_back(maze_motion_plan);


        // End Composite Instruction (from maze end back to start waypoint)
    ...
    void plan(const maze_msgs::srv::GenerateMotionPlan::Request::SharedPtr req,
                maze_msgs::srv::GenerateMotionPlan::Response::SharedPtr res)
    {
        try
        {
        ...
        const std::string& base_frame = req->tool_path.header.frame_id;
        ...
        plan_req.instructions = createProgram(manip_info, fromMsg(req->tool_path));
    ...
    rclcpp::Service<maze_msgs::srv::GenerateMotionPlan>::SharedPtr server_;
};
...
int main(int argc, char** argv)
{
    ...
    rclcpp::Node::SharedPtr node = std::make_shared<rclcpp::Node>("maze_planning_server");
